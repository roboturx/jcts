import network, socket, machine, time, binascii, gc
try: import hashlib # Standart hashlib kütüphanesini dene
except: import uhashlib as hashlib # MicroPython sürümünü kullan

# VERSİYON: 2026-01-18.026 (V011 BAZLI - TÜM SATIRLAR AÇIKLAMALI)
print("--- SISTEM: V011 ACIKLAMALI SÜRÜM (V026) ---")

# --- 1. DONANIM YAPILANDIRMASI ---
# GPIO pinlerini (Röleleri) çıkış olarak tanımlıyoruz
röleler = {
    'r1': machine.Pin(14, machine.Pin.OUT), 'r2': machine.Pin(15, machine.Pin.OUT),
    'r3': machine.Pin(16, machine.Pin.OUT), 'r4': machine.Pin(17, machine.Pin.OUT),
    'r5': machine.Pin(18, machine.Pin.OUT), 'r6': machine.Pin(19, machine.Pin.OUT),
    'r7': machine.Pin(20, machine.Pin.OUT), 'r8': machine.Pin(21, machine.Pin.OUT)
}

# Aynı anda çalışmaması gereken (Zıt yönlü) röle çiftleri
ZIT = {'r1':'r2','r2':'r1','r3':'r4','r4':'r3','r5':'r6','r6':'r5'}

# Süreli çalışma için her rölenin kapanma zamanını tutan sözlük
bitis = {k: 0 for k in röleler.keys()}

# Acil durumlarda veya başlangıçta tüm röleleri kapatan fonksiyon
def stop_all():
    for k in röleler.keys():
        röleler[k].value(0)
        bitis[k] = 0
stop_all()

# --- 2. NETWORK (WIFI) AYARLARI ---
ap = network.WLAN(network.AP_IF) # Access Point (Erişim Noktası) modunu seç
ap.active(False); time.sleep(0.5) # Temiz bir başlangıç için önce kapat
ap.config(essid="Vinc_Kumanda_Final", password="password123") # Ağ adı ve şifre
ap.ifconfig(('192.168.4.1', '255.255.255.0', '192.168.4.1', '8.8.8.8')) # Sabit IP ata
ap.active(True) # Ağı yayına aç

# WebSocket el sıkışması (handshake) için gerekli anahtar oluşturucu
def get_accept(key):
    d = hashlib.sha1((key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11").encode()).digest()
    return binascii.b2a_base64(d).decode().strip()

# --- 3. WEB ARAYÜZÜ (HTML / CSS / JS) ---
def web_page():
    return """<!DOCTYPE html><html><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no">
    <style>
        /* CSS: Görsel Tasarım ve Animasyonlar */
        @keyframes blink { 0% {background-color:#7f8c8d} 50% {background-color:#e74c3c} 100% {background-color:#7f8c8d} }
        body{font-family:sans-serif;background:#111;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;overflow:hidden;touch-action:none;-webkit-user-select:none;user-select:none}
        .s-wra{display:flex;gap:15px}
        .kum{background:#f1c40f;padding:15px;border-radius:25px;width:220px;text-align:center;border:4px solid #d4ac0d}
        .izg{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}
        .tus{width:70px;height:70px;background:#222;border:3px solid #333;color:#fff;font-size:24px;border-radius:50%;display:flex;align-items:center;justify-content:center}
        .akt{background:#e67e22!important;border-color:#fff} /* Tuşa basılınca turuncu yanar */
        .inp{width:40px;background:#333;color:#fff;border:1px solid #555;border-radius:4px;text-align:center}
        .st-t{width:26px;height:60vh;background:#000;border-radius:5px;display:flex;flex-direction:column-reverse;justify-content:space-between;padding:2px;border:1px solid #333}
        .bar{width:100%;height:8.5%;background:#1a1a1a}.l-g{background:#2ecc71}
        #b7{background:#e74c3c;font-weight:bold} /* E-STOP Butonu Kırmızı */
        #b7.pasif{animation: blink 2s step-end infinite !important; border-color:#fff !important}
        .estop-green{background-color:#2ecc71!important; border-color:#27ae60!important}
    </style></head>
    <body>
        <div class="s-wra">
            <div id="gv" class="kum">
                <div id="inf" style="font-size:10px;color:#000;background:#fff;border-radius:5px;padding:2px;margin-bottom:5px">BAGLI</div>
                <div class="izg">
                    <div class="gr"><div id="b1" class="tus" data-r="r1">▲</div><input type="number" id="s1" class="inp"></div>
                    <div class="gr"><div id="b2" class="tus" data-r="r2">▼</div><input type="number" id="s2" class="inp"></div>
                    <div class="gr"><div id="b3" class="tus" data-r="r3">▲</div><input type="number" id="s3" class="inp"></div>
                    <div class="gr"><div id="b4" class="tus" data-r="r4">▼</div><input type="number" id="s4" class="inp"></div>
                    <div class="gr"><div id="b5" class="tus" data-r="r5">▲</div><input type="number" id="s5" class="inp"></div>
                    <div class="gr"><div id="b6" class="tus" data-r="r6">▼</div><input type="number" id="s6" class="inp"></div>
                    <div id="b7" class="tus" style="border-radius:10px;font-size:14px">E-STOP</div>
                    <div id="b8" class="tus" style="border-radius:10px">💡</div>
                </div>
            </div>
            <div id="st" class="st-t"></div> </div>
        <script>
            let ws,con=false;const gv=document.getElementById('gv'),inf=document.getElementById('inf'),st=document.getElementById('st'),b7=document.getElementById('b7');
            for(let i=1;i<=10;i++)st.innerHTML+='<div class="bar l-g" id="br'+i+'"></div>';
            
            // WebSocket Bağlantı Fonksiyonu
            function cn(){
                ws=new WebSocket('ws://'+location.host+'/ws');
                ws.onopen=()=>{con=true;inf.innerText='SISTEM AKTIF';};
                ws.onclose=()=>{con=false;inf.innerText='KESILDI';setTimeout(cn,1500)};
            }

            // Tuş Atamaları ve Olay Dinleyiciler
            function setup(id){
                const el=document.getElementById(id),rid=el.dataset.r,inp=document.getElementById('s'+id.slice(1));
                el.onpointerdown=(e)=>{ // Tuşa basıldığında
                    e.preventDefault();if(!con||b7.classList.contains('pasif'))return;
                    const s=parseFloat(inp?.value)||0;el.classList.add('akt');
                    ws.send(rid+':1:'+s); // Örn: "r1:1:2.5" (Röle 1, Aç, 2.5 sn)
                };
                el.onpointerup=()=>{ // Tuş bırakıldığında
                    if(!parseFloat(inp?.value)){el.classList.remove('akt');ws.send(rid+':0:0')}
                };
            }
            for(let i=1;i<=6;i++)setup('b'+i);
            
            // E-STOP ve Işık Butonu Özel Kontrolleri
            b7.onpointerdown=()=>{let k=b7.classList.toggle('pasif');gv.classList.toggle('estop-green',k);ws.send('r7:'+(k?1:0)+':0');};
            document.getElementById('b8').onclick=(e)=>{if(!b7.classList.contains('pasif'))ws.send('r8:'+(e.target.classList.toggle('akt')?1:0)+':0')};
            cn();
        </script>
    </body></html>"""

# --- 4. WEBSOCKET VERİ İŞLEME ---
def handle_ws(c, addr):
    c.settimeout(0.01); last = time.ticks_ms()
    try:
        while True:
            now = time.ticks_ms()
            if time.ticks_diff(now, last) > 7000: break # 7 saniye veri gelmezse bağlantıyı kopar
            
            # Süresi dolan röleleri otomatik kapat
            for r, t in bitis.items():
                if t != 0 and time.ticks_diff(t, now) <= 0:
                    röleler[r].value(0); bitis[r] = 0
            
            try:
                p = c.recv(1024) # WebSocket paketini al
                if not p: break
                last = now
                # WebSocket Maskeleme Çözümü (Standart WS Protokolü)
                m, y = p[2:6], p[6:]; msg = "".join([chr(y[i] ^ m[i%4]) for i in range(len(y))])
                
                if ':' in msg:
                    r, v, s = msg.split(':'); val, sn = int(v), float(s)
                    
                    if r == 'r7': # E-STOP mantığı
                        if val == 1: stop_all()
                        röleler['r7'].value(val); continue
                    
                    if röleler['r7'].value() == 1: continue # E-STOP aktifken işlem yapma
                    
                    if val == 1: # Röleyi aç
                        if r in ZIT and röleler[ZIT[r]].value(): # Zıt yön açıksa kapat
                            röleler[ZIT[r]].value(0); bitis[ZIT[r]]=0
                        röleler[r].value(1)
                        # Eğer süre girilmişse bitiş zamanını hesapla
                        bitis[r] = time.ticks_add(now, int(sn*1000)) if sn > 0 else 0
                    else: # Röleyi kapat (Eğer süreli çalışma değilse)
                        if not bitis[r]: röleler[r].value(0)
            except: pass
            time.sleep(0.01)
    finally:
        c.close(); gc.collect()

# --- 5. ANA SUNUCU DÖNGÜSÜ ---
def run():
    s = socket.socket()
    s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind(('', 80)); s.listen(1)
    while True:
        gc.collect() # Bellek temizliği
        try:
            cl, ad = s.accept(); cl.settimeout(1.0)
            req = cl.recv(1024).decode()
            if 'Upgrade: websocket' in req: # WebSocket isteği mi?
                key = [l.split(':')[1].strip() for l in req.split('\r\n') if 'Sec-WebSocket-Key:' in l][0]
                cl.send("HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\nSec-WebSocket-Accept: "+get_accept(key)+"\r\n\r\n")
                handle_ws(cl, ad) # WebSocket moduna geç
            else: # Standart Web Sayfası isteği
                cl.send("HTTP/1.1 200 OK\r\nContent-Type: text/html\r\nConnection: close\r\n\r\n" + web_page())
                cl.close()
        except: pass

# Anakod bunu yap
run()

# ==========================================================
# KODUN ANA ÖZELLİKLERİ VE ÇALIŞMA MANTIĞI
# ==========================================================
# 1. GERÇEK ZAMANLI KONTROL: WebSocket protokolü kullanılarak 
#    gecikmesiz (low-latency) iletişim sağlanır.
# 2. GÜVENLİK (ZIT YÖN KORUMASI): Örneğin Yukarı ve Aşağı 
#    rölelerinin aynı anda çekmesi yazılımsal olarak engellenmiştir.
# 3. SÜRELİ ÇALIŞMA: Arayüzdeki kutulara girilen saniye (sn) 
#    kadar röle aktif kalır, süre sonunda otomatik durur.
# 4. E-STOP (ACİL DURDURMA): r7 rölesine bağlı E-STOP aktifleştiğinde 
#    tüm işlemler durur ve sistem komut kabul etmez.
# 5. DİNAMİK ARAYÜZ: Pointer olayları ile hem mobil hem masaüstü 
#    tarayıcılarda kayıpsız dokunma hassasiyeti sunar.
# 6. BELLEK YÖNETİMİ: 'gc.collect()' ile kısıtlı RAM'e sahip 
#    mikrokontrolcülerde şişme yapmadan stabil çalışır.
# ==========================================================
