...
# WebSocket el sıkışması için şifreleme kütüphanesini dene
try: import hashlib
...
# Birbirine zıt çalışan (yukarı-aşağı gibi) röle çiftlerini tanımla
...
print("\n" + "="*40)
print("     VINC OTOMASYON SISTEMI V046")
print("="*40)
print("WEB ADRES  : http://{}".format(IP_CFG[0]))
print("SINYAL     : 100 KADEME / 2ms (YESIL UST)")
print("GUVENLIK   : FAIL-SAFE & ZIT KORUMA")
print("="*40 + "\n")
...
def web_page():
    ...
    <body>
        ...
        <script>
            ...
            /* Renk Paleti: Alttan Üste (Kırmızı -> Yeşil) */
            const getCol=(i)=>{
                if(i < 20) return '#b71c1c'; if(i < 40) return '#e67e22';
                if(i < 60) return '#f1c40f'; if(i < 80) return '#1b5e20';
                return '#2ecc71';
            };
            /* 100 barı HTML'e ekle */
            ...
            function cn(){
                ...
                ws.onmessage=(e)=>{
                    if(e.data==='PONG'){
                        let lat=Date.now()-pT;
                        /* 2ms gecikme başına 1 bar düşür. Full sinyal = 100 bar */
...
def handle_ws(c, addr):
    ...
    try:
        while True:
            now = time.ticks_ms()
            if time.ticks_diff(now, last) > 3000: break # Fail-Safe Koruması
